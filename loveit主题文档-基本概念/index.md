...
直接安装满足你操作系统 (**Windows**, **Linux**, **macOS**) 的最新版本 [:(far fa-file-archive fa-fw): Hugo (> 0.62.0)](https://github.com/gohugoio/hugo/releases).

将程序解压放置于软件文件夹下, 并将该路径添加到环境变量中.
...
{{< admonition >}}
当一个工程中有`git submodule add`的子模块时，执行git clone并不会自动下载子模块的内容，此时需执行如下命令进行初始化:
...
### 3.2 网站图标, 浏览器配置, 网站清单 {#website-icon}
