...
只有在文章的参数和你的 [网站设置](../loveit主题文档-基本概念#site-configuration) 中的 `page` 部分不一致时才有必要这么做.
{{< /admonition >}}
...
* **date**: 这篇文章创建的日期时间. 它通常是从文章的前置参数中的 `date` 字段获取的, 但是也可以在 [网站配置](../loveit主题文档-基本概念#site-configuration) 中设置.
* **lastmod**: 上次修改内容的日期时间.
...
* **toc**: {{< version 0.2.9 changed >}} 和 [网站配置](../loveit主题文档-基本概念#site-configuration) 中的 `params.page.toc` 部分相同.
* **code**: {{< version 0.2.0 >}} 和 [网站配置](../loveit主题文档-基本概念#site-configuration) 中的 `params.page.code` 部分相同.
* **math**: {{< version 0.2.0 changed >}} 和 [网站配置](../loveit主题文档-基本概念#site-configuration) 中的 `params.page.math` 部分相同.
* **mapbox**: {{< version 0.2.0 >}} 和 [网站配置](../loveit主题文档-基本概念#site-configuration) 中的 `params.page.mapbox` 部分相同.
* **share**: 和 [网站配置](../loveit主题文档-基本概念#site-configuration) 中的 `params.page.share` 部分相同.
* **comment**: {{< version 0.2.0 changed >}} 和 [网站配置](../loveit主题文档-基本概念#site-configuration) 中的 `params.page.comment` 部分相同.
* **library**: {{< version 0.2.7 >}} 和 [网站配置](../loveit主题文档-基本概念#site-configuration) 中的 `params.page.library` 部分相同.
* **seo**: {{< version 0.2.10 >}} 和 [网站配置](../loveit主题文档-基本概念#site-configuration) 中的 `params.page.seo` 部分相同.
...
你可以通过在 [网站配置](../loveit主题文档-基本概念#site-configuration) 中设置 `summaryLength` 来自定义摘要长度.

如果您要使用 [CJK]^(中文/日语/韩语) 语言创建内容, 并且想使用 Hugo 的自动摘要拆分功能，请在 [网站配置](../loveit主题文档-基本概念#site-configuration) 中将 `hasCJKLanguage` 设置为 `true`.
...
这部分内容在 [Markdown 基本语法页面](../markdown-基本语法/) 中介绍.
...
这部分内容在 [Emoji 支持页面](../emoji-支持/) 中介绍.

### 数学公式

**LoveIt** 基于 [$ \KaTeX $](https://katex.org/) 提供数学公式的支持.

在你的 [网站配置](../loveit主题文档-基本概念#site-configuration) 中的 `[params.math]` 下面设置属性 `enable = true`,
并在文章的前置参数中设置属性 `math: true`来启用数学公式的自动渲染.

{{< admonition >}}
**LoveIt**默认采用的是较老的 $\KaTeX @ 0.11.1$ 版本, 有许多语法都不支持, 建议换成[最新版本](https://github.com/KaTeX/KaTeX/releases).

更换方法为将`assets/data/cdn/jsdelivr.yml`中的`katex@0.11.1`全部替换成最新的版本号.
{{< /admonition >}}
...
{{< admonition >}}
对于多行公式，不能使用`\\`作为换行符，作为替代，矩阵内部换行可以用`\cr`；或者使用转义形式`\\\\`，并且之后不能换行。

如果不想改变 $ \KaTeX $ 的习惯写法，也可以使用`<div></div>`将公式包裹起来。
例如：
...
{{< admonition tip >}}
你可以在 [网站配置](../loveit主题文档-基本概念#site-configuration) 中自定义公式块和行内公式的分割符.
{{< /admonition >}}
...
在你的 [网站配置](../loveit主题文档-基本概念#site-configuration) 中的 `[params.math]` 下面设置属性 `copyTex = true` 来启用 Copy-tex.
...
在你的 [网站配置](../loveit主题文档-基本概念#site-configuration) 中的 `[params.math]` 下面设置属性 `mhchem = true` 来启用 mhchem.
